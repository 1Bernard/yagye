  ...
  module Payouts
    class FilterView < ApplicationComponent
      ...
      STATES = [
        ...
        [ "Paid",         "paid",      :check_circle ],
        [ "Failed",       "failed",    :alert_circle ]
      ].freeze
      ...
      def view_template
        turbo_frame_tag "drawer-frame" do
          div(class: DRAWER_HEAD) do
            div do
              p(class: TYPE_TITLE)   { plain "Filter payouts" }
              p(class: "#{TYPE_CAPTION} mt-[3px]") { plain "Narrow by state or date." }
            end
          ...
          form(action: payouts_path, method: "get",
            ...
            div(class: "flex-shrink-0 flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-white") do
              a(href: payouts_path(q: @query.presence),
