defmodule YagyeCore.Repo.Migrations.CreatePayments do
  ...
  def change do
    ...
    create constraint(:payments, :valid_state,
             check:
               "state IN ('created','processing','requires_action','authorised','succeeded','failed','cancelled','indeterminate')"
           )

    create constraint(:payments, :valid_rail, check: "rail IN ('fiat_provider','internal')")
    create constraint(:payments, :valid_mode, check: "mode IN ('simulation','live')")
    create constraint(:payments, :positive_amount, check: "amount > 0")
