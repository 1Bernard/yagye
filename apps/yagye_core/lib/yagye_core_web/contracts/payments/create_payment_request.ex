defmodule YagyeCoreWeb.Contracts.Payments.CreatePaymentRequest do
  ...
  OpenApiSpex.schema(%{
    ...
    type: :object,
    properties: %{
      amount: %Schema{type: :integer, description: "Amount in minor units (e.g. pesewas)", minimum: 1},
      currency: %Schema{type: :string, description: "ISO 4217 currency code", minLength: 3, maxLength: 3},
      rail: %Schema{type: :string, description: "Payment rail", enum: ["fiat_provider", "internal"]},
      method: %Schema{type: :string, description: "Payment method (e.g. card, mobile_money)"},
      merchant_reference: %Schema{type: :string, description: "Merchant-supplied idempotency reference"},
