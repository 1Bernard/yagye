defmodule YagyeCoreWeb.FallbackController do
  ...
  def call(conn, {:error, :invalid_state}),
    do: Response.unprocessable(conn, "invalid_state", "Resource is not in a state that allows this operation")

  def call(conn, {:error, :invalid_onboarding_state}),
    do: Response.unprocessable(conn, "invalid_onboarding_state", "Onboarding is not in a submittable state")
